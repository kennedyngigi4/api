...
def process_listing_image(image_id):
    ...
    # ---- Add watermark (no resizing, no blur) ----
    ...
    if not os.path.exists(font_path):
        ...
        font2 = ImageFont.truetype(font_path, max(int(min(img.size) * 0.06), 24))
    ...
    draw.text(
        ...
        line1, font=font1, fill=(0, 0, 0, 100),
        stroke_width=2, stroke_fill=(255, 255, 255, 160)
    )
    draw.text(
        (center_x - text2_w // 2, start_y + text1_h + 25),
        line2, font=font2, fill=(0, 0, 0, 100),
        stroke_width=2, stroke_fill=(255, 255, 255, 160)
    )

    final = Image.alpha_composite(img, watermark_layer).convert("RGB")

    # ---- Save processed main image (no resizing, just compress) ----
    buffer = BytesIO()
    final.save(buffer, format="JPEG", quality=90, optimize=True, progressive=True)
    buffer.seek(0)
    ...
    # Add sharpening for clarity
    thumb = thumb.filter(ImageFilter.UnsharpMask(radius=1.5, percent=130, threshold=2))

    thumb_buffer = BytesIO()
    thumb.save(thumb_buffer, format="JPEG", quality=92, optimize=True, progressive=True)
    thumb_buffer.seek(0)
    ...
    print("✅ Clean image + thumbnail saved:", instance.thumbnail.url if instance.thumbnail else "❌ Missing")
